...
本章节介绍 Univer 公式引擎的整体架构，建议在阅读本章节之前，先阅读[整体架构](/zh-cn/guides/concepts-and-architecture/architecture)。

公式引擎设计的主要目标如下：

1. 支持 Univer 的不同文档类型和相关功能接入公式
2. 提供快速的使用体验，支持和 [web worker](../web-worker/) 和服务端计算
...
<Image alt='formula engine architecture' src={formulaEngineArchitecture} width='500' />
