...
:::tip
本文档仅适用于 📊 Univer Sheet。
:::

## 单元格位置

Univer Sheet 中的单元格数据存储在 [IWorksheetData](/api/core/interfaces/IWorksheetData.html) 的 [cellData](/api/core/interfaces/IWorksheetData.html#cellData) 字段中，cellData 是一个二维 Map 结构，每个 key 值标记了单元格的位置，value 是一个 [ICellData](/api/core/interfaces/ICellData.html) 对象，包含了单元格值、样式、类型等所有单元格信息。
...
详细了解 [IStyleData](/api/core/interfaces/IStyleData.html) 每个字段的类型信息。
...
`cellData.p` 是一个对象，表示富文本，同时也是一个 Univer Doc。详细了解 [IDocumentData](/api/core/interfaces/IDocumentData.html)。
