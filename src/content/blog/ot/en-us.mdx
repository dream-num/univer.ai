...
The concept of Univer's unbounded documents allows the mixing of multiple document types within the same unbounded space. For example, spreadsheets, documents, and slides can coexist in a single unbounded space. Therefore, the collaborative scheduling algorithm cannot rely on document-type-specific characteristics, such as the row-column structure of spreadsheets. Instead, it needs to be applicable to any document type. To achieve this, we have implemented a document-type-agnostic collaborative scheduling algorithm. This algorithm handles network connectivity, revision history, offline editing, and other related issues. Each document type that integrates with collaborative editing needs to design its own data structure for operations and implement operation transformation algorithms. This significantly reduces the development complexity of collaborative editing for different document types.
...
We hope that this article has helped you gain a better understanding of collaborative editing. If you have any questions, we welcome you to discuss them with us in the comments.
