---
title: Quickstart
---

If you want to get started with Univer as quickly as possible, you've come to the right place! This page will guide you through building a working Univer app in just a few minutes.

{/* ## Univer in 60 seconds */}

{/* 这里需要一个视频 */}

## Play Online

You can play with Univer online without installing anything. Just click the link below and start using Univer right away.

import PlaygroundBtns from './_components/PlaygroundBtns.astro'

<PlaygroundBtns
  items={[{
    color: 'rgb(73, 184, 17)',
    icon: '📊',
    title: 'Univer Sheet',
    href: 'https://stackblitz.com/~/github.com/awesome-univer/sheets-vite-demo',
  }, {
    color: 'rgb(11, 158, 251)',
    icon: '📝',
    title: 'Univer Doc',
    href: 'https://stackblitz.com/~/github.com/awesome-univer/docs-vite-demo',
    type: 'beta'
  }, {
    color: 'rgb(254, 75, 75)',
    icon: '🎨',
    title: 'Univer Slide',
    type: 'dev',
  }]}
/>

## Starter Kit

If you prefer to start with a local development environment, you can use the following starter kits to create a new Univer app in just a few seconds. We provide two starter kits for different types of apps:

import { Tabs, TabItem } from "@astrojs/starlight/components";

<Tabs>
  <TabItem label="npm">
    ```shell
      # 📊 create a univer sheet app
      npx degit dream-num/vite-sheet-template [your-app-name]

      # 📝 create a univer doc app
      npx degit dream-num/vite-doc-template [your-app-name]
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```shell
      # 📊 create a univer sheet app
      pnpm dlx degit dream-num/vite-sheet-template [your-app-name]

      # 📝 create a univer doc app
      pnpm dlx degit dream-num/vite-doc-template [your-app-name]
    ```
  </TabItem>
</Tabs>

Great job on your first steps with Univer! You've successfully created your first project and gotten a taste of what Univer can do.

## Next Steps

Congratulations on making it this far! You're well on your way to mastering Univer. Here are some next steps to help you continue your journey:

- **🔰 Getting Started**: If you want to learn how to manually set up a Univer project from scratch, check out the [Getting Started](/guides/getting-started) chapter. It will guide you through the process step by step.
- **🏗️ Advanced Use**: Ready to take your Univer skills to the next level? The [Advanced Use](/guides/advanced-use) chapter covers more advanced topics and techniques for working with Univer.
- **📚 Tutorials**: Looking for practical examples and real-world use cases? Head over to the [Tutorials](/guides/tutorials) chapter, where you'll find a collection of hands-on tutorials that demonstrate how to build various applications and features with Univer.
- **🧱 Concepts & Architecture**: Want to dive deep into the inner workings of Univer? The [Concepts & Architecture](/guides/concepts-and-architecture/architecture/) chapter provides a comprehensive overview of Univer's architecture, core concepts, and design principles.
- **🪄 Customizing Univer**: Interested in extending and customizing Univer to fit your specific needs? The [Customizing Univer](/guides/customization) chapter is your go-to resource for learning how to create custom plugins, themes, and extensions.
- **🎢 Playground**: Don't forget to explore the [Playground page](/playground), where you can experiment with various Univer demos and see the library in action.

No matter which path you choose, Univer is here to support you every step of the way. Happy coding!
