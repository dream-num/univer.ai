...
export class RequestService implements IRequestService, IDisposable {
  ...
  async import(importRequest: IImportRequest) {
    ...
    try {
      ...
      const response = await this._httpService.post(url, options) as HTTPResponse<IImportResponse>
      const data = response.body
  ...
  async getTask(taskID: string): Promise<IGetTaskResponse | null> {
    return new Promise<IGetTaskResponse | null>((resolve, reject) => {
      try {
        ...
        const poll = async () => {
          const response = await this._httpService.get(url) as HTTPResponse<IGetTaskResponse>
          const data = response.body
  ...
  async signUrl(fileID: string) {
    try {
      ...
      const response = await this._httpService.get(url) as HTTPResponse<ISignUrlResponse>
      const data = response.body
  ...
  async getUrlContent(url: string) {
    try {
      ...
      const response = await this._httpService.get(url, options) as HTTPResponse<string>
      const data = response.body
