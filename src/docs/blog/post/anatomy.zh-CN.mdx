...
<Callout type="info" emoji="💡">
  请阅读 [扩展命令](guides/sheet/advanced/custom-command) 了解如何自定义包括 mutation 在内的命令。
</Callout>
...
Univer 的菜单项以一个 `IMenuItem` 描述，筛选插件的菜单项定义在 [sheets-filter.menu.ts](https://github.com/dream-num/univer/blob/dev/packages/sheets-filter-ui/src/controllers/sheets-filter.menu.ts) 当中。
...
筛选插件的快捷键定义在 [sheets-filter.shortcut.ts](https://github.com/dream-num/univer/blob/dev/packages/sheets-filter-ui/src/controllers/sheets-filter.shortcut.ts) 中。
