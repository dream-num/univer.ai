...
在设计 Univer 的架构的过程中，我们团队引入了依赖注入（Dependency Injection, DI）来应对上面所列举出的复杂度。这篇文章将介绍 DI 在 Univer 中的实践。
...
DI 有一定的学习门槛，使用者需要掌握控制反转和依赖倒置的概念，才能充分理解 DI 所带来的优势。此外，还需学习装饰器语法，甚至了解 TypeScript 的一些配置，才能在项目中有效应用 DI。这可能会让部分用户感到畏惧，Angular 就是一个例子。对此，我们致力于完善我们的 [Facade API](https://univer.ai/zh-CN/guides/sheet/facade/facade)，尽量将 DI 隐藏起来，为用户提供一个更简单、基于配置的接口，从而大幅降低 Univer 的入门门槛。
