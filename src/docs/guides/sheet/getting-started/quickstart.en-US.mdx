...
<Steps>
  ...
  <Tabs items={['pnpm', 'npm', 'Without package manager']}>
    <Tabs.Tab label="pnpm">
      ...
      ```
      <Callout emoji="⚠️">
        If you deploy the server on a different machine, you also need to modify the `server` configuration in `vite.config.ts`, changing `localhost` to the server's IP address.
      </Callout>
