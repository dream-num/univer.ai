...
<Callout>
  如果你是首次使用 Univer，我们推荐从 `@univerjs/presets` 开始。在后续的小节中，除非特殊说明，我们都会假设你使用了 Presets。如果你已经较为熟悉 Univer，并且希望更加灵活地引入插件（例如懒加载部分插件），可以参考 [高级安装和引入](/guides/sheet/advanced/installation)。
</Callout>
...
TODO: @jikkai update imports and API here
...
我们这里所调用的 `univerAPI` 对象被称为 Univer 的 Facade API（面板 API）。
...
通过调用当前 `FWorkbook` 的 `save` 方法，可以得到 `IWorkbookData` 对象，包含了表格内部的数据。
