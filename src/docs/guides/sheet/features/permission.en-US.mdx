...
Execute the corresponding permission settings by driving command
...
```
- workbook permission
Currently, the overall permissions of a workbook are not set through mutations, but are modified directly through the API. Therefore, we provide a FacadeAPI. On the facade instance, we expose an API that can adjust the edit permissions of a workbook. You can set the edit permissions of a workbook by calling this API.
```typescript
