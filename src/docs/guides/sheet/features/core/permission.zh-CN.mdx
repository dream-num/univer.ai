...
目前工作簿的整体权限是直接通过 API 来修改权限点位，我们提供了 [Facade API](/guides/sheet/facade/facade)，你可以通过调用这个 API 来设置工作簿的编辑权限。
...
这个 demo 是给自定义选区添加权限保护，这里面的参数和上面的一样，增加了 `ranges` 参数，这个参数是你选中的区域， 还有一个 `id` 参数, 这个参数是用来标识这个规则的唯一 id，用来删除这个规则。   
