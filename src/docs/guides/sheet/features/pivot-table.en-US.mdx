...
import data from './pivot-table/data.txt'

# Pivot Table

<BadgeGroup values={[UniverTypes.SHEET]} value={UniverTypes.SHEET} />

<Sandbox entryFile={entryFile} files={{'data.ts':data}} />
...
- `@univerjs-pro/sheets-pivot` assembles pivot-engine into universal sheet plugin to support table rendering, collaboration, formulas and other capabilities
...
<Tabs items={['pnpm', 'npm']}>
  <Tabs.Tab label="pnpm">
    ```shell
    pnpm add @univerjs-pro/pivot-engine @univerjs/sheets-pivot @univerjs/sheets-pivot-ui
    ```
  </Tabs.Tab>
  <Tabs.Tab label="npm">
    ```shell
    npm install @univerjs-pro/pivot-engine @univerjs/sheets-pivot @univerjs/sheets-pivot-ui
    ```
  </Tabs.Tab>
</Tabs>
