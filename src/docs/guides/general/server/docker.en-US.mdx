...
## Offline Deployment <a id="offline-deployment"></a>
...
1. [Click here to download the offline installation package](https://univer.ai/releases/univer-server/download) and extract it on your server.

```bash
tar -xvf installation-package.tar.gz # Replace 'installation-package' with the actual file name
```

2. Navigate to the extracted directory and run `bash install.sh` to start the service.
...
4. Verify that the service is available.
