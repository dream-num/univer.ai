...
- USIP Client: The end that initiates USIP requests, generally is our Univer Server
- USIP Server: The end that implements USIP and provides services, generally is Third-party Server
...
![relationship](./usip/usip-relationship.png)
![api call flowchart](./usip/usip-flowchart.png)
...
2. Implementation: Upon successfully creating a document by calling Univer Server, it returns a `unitID` as document ID. The integrating party needs to store this ID to manage their own document management operations effectively.
