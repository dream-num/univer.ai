...
- USIP Client: The end that initiates USIP requests, generally Univer Server
- USIP Server: The end that implements USIP and provides services, generally a Third-party Server
...
In this way, while ensuring that data remains within the third-party client system, Univer Server can successfully integrate into the client's system.

![relationship](./usip/AzQSbQDVuoKkRZxrRPqcHtF0nsh.png)
![api call flowchart](./usip/NdzmbFpnio4mQexalKBcw7Hwnmf.png)
...
2. Implementation: Upon successfully creating a document by calling Univer Server, it returns a `unitID` document ID. The integrating party needs to store this ID to manage their own document management operations effectively.
...
```shell
curl -X GET "http://localhost:8080/userinfo?userIDs=1&userIDs=2"
...
```shell
curl -X GET "http://localhost:8080/collaborators?unitIDs=AA&unitIDs=xx"
