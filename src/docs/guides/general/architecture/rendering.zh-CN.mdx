...
![render unit architecture](./rendering/render-unit-architecture.png)
...
除了 `IRenderContext`，`IRenderModule` 可以通过依赖注入获取其他 `IRenderModule`。同时由于 `RenderUnit` 中的 `Injector` 是全局 `Injector` 的子节点，因此 `IRenderModule` 中也可以注入全局模块。

某些情形下，你可能会需要从全局模块获取一个 `IRenderModule`，这种情形下你可以使用如下方法：

```ts
const renderManagerService = this._injector.get(IRenderManagerService);
const renderer = renderManagerService.getRenderById(unitId);
render.with(DependencyToken);
```
