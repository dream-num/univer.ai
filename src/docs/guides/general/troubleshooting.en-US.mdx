...
### I'm using React but I'm running into some issues

First, make sure your React version is 16.9 or higher, as Univer's UI layer depends on React and uses React Hooks.

In addition, there is a known issue with the DI library `@wendellhu/redi` that Univer depends on, which does not currently support React's StrictMode. You can resolve this issue by removing StrictMode.
