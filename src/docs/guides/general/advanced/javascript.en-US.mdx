...
`@wendellhu/redi` provides the `setDependency` method to explicitly declare dependencies in JavaScript. For example:

```diff
import { Plugin, ICommandService, UniverInstanceType } from '@univerjs/core'
- import { Inject, Injector } from '@wendellhu/redi'
+ import { Injector, setDependency } from '@wendellhu/redi'
...
+ setDependency(MyPlugin, [Injector, ICommandService])
```
