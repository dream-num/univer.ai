...
### Subscribe to the cell's click event

```ts
const activeWorkbook = univerAPI.getActiveWorkbook();
activeWorkbook.onCellClick((selection) => {
  // selection is a FRange object. You can use FRange to get information such as whether the range is merged and the cell coordinates.
